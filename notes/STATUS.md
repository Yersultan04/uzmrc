...
## 🟢 Сессия 2026-06-16 — token-aware суб-батчинг эмбеддингов + тесты
...
1. **⭐ Embedding-кэш (Tier 0, наивысший ROI)** — `sha256(provider+model+text) → vector` в Postgres. Эмбеддинг считается ОДИН раз; реиндекс того же корпуса → мгновенно и без сжигания квоты. Снимает главную боль free-эмбеддера (вчера 4 реиндекса = 4× квота). См. `notes/zero-budget-playbook.md`.
2. **Демо для клиента:** прогнать `/compare` на «чужом» приказе (не из корпуса), показать вердикты conflict/addition/gap, а не только duplicate. Бюджет Cerebras ~1M ток/день (≈9 compare).
